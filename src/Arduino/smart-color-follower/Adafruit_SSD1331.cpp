...
//Serial.println("fillRect");
  ...
  delayMicroseconds(SSD1331_DELAYS_HWFILL_MICROS * h * w / (94*64));
}
...
void Adafruit_SSD1331::writeFastVLine(int16_t x, int16_t y,
...
void Adafruit_SSD1331::writeFastHLine(int16_t x, int16_t y,
...
void Adafruit_SSD1331::begin(void) {
    ...
    // set pin directions (BKE edit: set this after spi.begin so we can use the RS pin where MISO usually is, since we don't use MISO
